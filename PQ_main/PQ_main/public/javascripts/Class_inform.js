        ...
        for (let session = 0; session < this._question.length; ++session) {            
            ...
            for (var item of this._question[session]) { //size direction
                ...
                await this._generateAnswer(item[1] + 1, 5000).then((data) => {//5000
                    this._one += data[0];
                    this.correct = data[1];
                    this.correct == 0 ? timer[garbo[0]] =timer[garbo[0]]+ (Math.abs(last_timer[garbo[0]]- timer[garbo[0]]) * 0.1 + 1) : 
                    timer[garbo[0]]= timer[garbo[0]]- (Math.abs(last_timer[garbo[0]] -timer[garbo[0]]) * 0.1 + 1);
                    timer[garbo[0]]+=15;
                    if (timer[garbo[0]] < 1) {
                        timer[garbo[0]] = 1;
                    }
